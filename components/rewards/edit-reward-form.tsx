...
const rewardsData = [
  {
    ...
    validTo: "2025-12-31",
    includedProducts: ["منتجات الملابس"],
    excludedProducts: ["منتجات مخفضة"],
    isActive: true,
  },
  {
    id: "2",
    name: "خصم 20 ريال",
    description: "خصم 20 ريال على طلبك التالي",
    type: "fixed_discount",
    pointsCost: 1000,
    minOrderValue: 200,
    usageLimit: 2,
...
export function EditRewardForm({ isOpen, onClose, onSave, rewardId }: EditRewardFormProps) {
  ...
  // تحميل بيانات المكافأة عند فتح النموذج
  useEffect(() => {
    if (isOpen && rewardId) {
      ...
      if (reward) {
        setFormData({
          ...
          includedProducts: reward.includedProducts.join(", "),
          excludedProducts: reward.excludedProducts.join(", "),
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px] rounded-[16px]" dir="rtl">
        <DialogHeader>
          ...
          <DialogDescription>
            قم بتعديل تفاصيل المكافأة. يمكنك التنقل بين الأقسام المختلفة لتعديل جميع المعلومات.
          </DialogDescription>
        </DialogHeader>
